...
import { X, Key, Database, RefreshCw, Save, Settings as SettingsIcon, BrainCircuit, Sparkles } from 'lucide-react';
...
interface AppSettings {
  ...
  iconSize: number;
  graphTheme: 'universe' | 'cyberpunk' | 'deep-sea';
}
  ...
  useEffect(() => {
    ...
    chrome.storage?.local.get([
      ...
      setSettings({
        ...
        iconSize: result.iconSize !== undefined ? (result.iconSize as number) : 1.0,
        graphTheme: (result.graphTheme as 'universe' | 'cyberpunk' | 'deep-sea') || 'universe',
      });
    });
  }, [isOpen]);
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto max-h-[500px]">
            <div className="space-y-6">
                ...
                <div className="space-y-4 pt-2 border-t border-slate-800/50">
                        <label className="text-sm font-bold text-slate-300 flex items-center gap-2">
                            <Sparkles size={16} className="text-blue-500/70" /> マップテーマ
                        </label>
                        <div className="grid grid-cols-3 gap-3">
                            <button
                                onClick={() => setSettings({ ...settings, graphTheme: 'universe' })}
                                className={`p-3 rounded-xl border text-left transition-all ${
                                    settings.graphTheme === 'universe' 
                                    ? 'bg-blue-600/10 border-blue-500 text-blue-400' 
                                    : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'
                                }`}
                            >
                                <div className="text-xs font-black">Universe</div>
                                <div className="text-[9px] mt-1 opacity-60">星空と銀河の宇宙</div>
                            </button>
                            <button
                                onClick={() => setSettings({ ...settings, graphTheme: 'cyberpunk' })}
                                className={`p-3 rounded-xl border text-left transition-all ${
                                    settings.graphTheme === 'cyberpunk' 
                                    ? 'bg-blue-600/10 border-blue-500 text-blue-400' 
                                    : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'
                                }`}
                            >
                                <div className="text-xs font-black">Cyberpunk</div>
                                <div className="text-[9px] mt-1 opacity-60">ネオンと回路の電脳</div>
                            </button>
                            <button
                                onClick={() => setSettings({ ...settings, graphTheme: 'deep-sea' })}
                                className={`p-3 rounded-xl border text-left transition-all ${
                                    settings.graphTheme === 'deep-sea' 
                                    ? 'bg-blue-600/10 border-blue-500 text-blue-400' 
                                    : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'
                                }`}
                            >
                                <div className="text-xs font-black">Deep Sea</div>
                                <div className="text-[9px] mt-1 opacity-60">深海と生命の静寂</div>
                            </button>
                        </div>
                    </div>
                </div>
